...
extension SignUpViewController{
    ...
    func checkData() -> Bool {
        ...
        if currentUser.phoneNumber.length() < 10{
            UIAlertController.showAlertOfStyle(UIAlertControllerStyle.Alert, Message: "Please Enter valid phone number", completion: nil)
            return false
        }
            ...
            if NetworkClass.isConnected(true) {
                ...
                NetworkClass.sendRequest(URL: Constants.URLs.signup, RequestType: .POST, Parameters: currentUser.getSignupDictionary(), Headers: nil, CompletionHandler: {
                    ...
                    if let responseDictionary = responseObj as? Dictionary<String, AnyObject>{
                        let msg = responseDictionary["message"] as? String ?? "Unable to Signup"
                        let created = (responseDictionary["created"] as? Bool) ?? false
                        if created{
                            UIAlertController.showAlertOfStyle(UIAlertControllerStyle.Alert, Title: "Signup Authentication", Message: msg, OtherButtonTitles: nil, CancelButtonTitle: "ok", completion: {UIAlertAction in
                                self.navigationController?.popViewControllerAnimated(true)})
                        }
                        else{
                            UIAlertController.showAlertOfStyle(UIAlertControllerStyle.Alert, Title: "Signup Authentication", Message: msg, OtherButtonTitles: nil, CancelButtonTitle: "ok", completion: nil)
                        }
                    }
                    self.hideLoader()
                })
